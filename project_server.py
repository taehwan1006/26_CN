...
설명: 본 프로그램은 저수준 TCP 소켓을 활용하여 HTTP/1.1 규격을 준수하는 웹 서버를 구현한 것입니다.
...
def run_server():
    ...
    print(f"[*] HTTP 서버 구동 완료 (포트 {PORT}) - DB 예외 처리 모드")
        ...
        try:
            while True:
                try:
                    ...
                    if not request_data:
                        break  # 클라이언트가 연결을 종료한 경우 루프 탈출
                except socket.timeout:
                    print(f"[-] 타임아웃(5초) 발생. 클라이언트 연결을 종료합니다: {addr}")
                    break # 5초 대기 후 요청 없으면 안전하게 탈출
            ...
            if path == "/api/users":
                ...
                with db_conn.cursor() as cursor:
                    ...
                    if method == "GET":
                        ...
                        display_text = f"총 {len(users)}명의 회원이 조회되었습니다.<br>"
                        display_text += "-" * 55 + "<br>"
                        for idx, u in enumerate(users, 1):
                            display_text += f"{idx:2d}. [{u['role']:<10}] {u['name']} ({u['user_id']}) |  {u['email']}<br>"
                        display_text += "-" * 55
                        
                        response = build_response("200 OK", display_text)
