...
![400 Bad Request](./images/400_bad_request.png)
*(위 경로에 400 에러 캡처 이미지를 삽입하세요)*

```text
============================================================
📡 [서버로 요청 전송] POST /api/users
📦 [전송 데이터] user_id=bad_user&email=bad@test&name=&role=STUDENT
------------------------------------------------------------
...
![201 Created and 409 Conflict](./images/201_409_conflict.png)
*(위 경로에 201/409 에러 캡처 이미지를 삽입하세요)*

```text
============================================================
📡 [서버로 요청 전송] POST /api/users
📦 [전송 데이터] user_id=taehwan_s&email=taehwan@kookmin.ac.kr&name=신태환&role=STUDENT
------------------------------------------------------------
...
권한 필터링(`?role=INSTRUCTOR`)을 적용한 GET 조회 결과이며, HTML이 완벽히 제거되어 콘솔에 예쁘게 정렬된 모습입니다.

![200 OK GET](./images/200_ok_get.png)
*(위 경로에 GET 조회 결과 캡처 이미지를 삽입하세요)*

```text
============================================================
📡 [서버로 요청 전송] GET /api/users?role=INSTRUCTOR
------------------------------------------------------------
✅ [서버 응답 상태] HTTP/1.1 200 OK
📄 [서버 응답 본문]
총 1명의 회원이 조회되었습니다.
-------------------------------------------------------
 1. [INSTRUCTOR] 김강사 (inst01) | ✉️ inst@test.com
-------------------------------------------------------
============================================================
```
...
![403 and 404](./images/403_404.png)
*(위 경로에 403/404 에러 캡처 이미지를 삽입하세요)*
...
📦 [전송 데이터] user_id=ghost999&name=수정완료
------------------------------------------------------------
